...
def enhancedFeatureExtractorDigit(datum):
    ...
    def visit(pixel):
        ...
        for direct in directs:
            adj = (pixel[0] + direct[0], pixel[1] + direct[1])
            if adj[0] < 0 or adj[1] < 0 or adj[0] >= DIGIT_DATUM_WIDTH or adj[1] >= DIGIT_DATUM_WIDTH:
                continue
            if not visited[adj] and not datum.getPixel(adj[0], adj[1]):
                visit(adj)

    for i in range(DIGIT_DATUM_WIDTH):
        for j in range(DIGIT_DATUM_HEIGHT):
            if not (visited[(i, j)] or datum.getPixel(i, j)):
                cnt += 1
                visit((i, j))

    features["concom_1"] = cnt > 1
    features["concom_2"] = cnt > 5
    features["concom_3"] = cnt > 3 
...
def enhancedPacmanFeatures(state, action):
    ...
    features["num_ghost"] = sum([(util.manhattanDistance(ghost.getPosition(), position) < 4) for ghost in ghosts])
    features["scare_times"] = min([(ghost.scaredTimer == 0) for ghost in ghosts])
    ...
    if len(capsules) > 0:
        features["capsules"] = min([util.manhattanDistance(capsule, position)
                                    for capsule in capsules]) < min([util.manhattanDistance(capsule, position)
